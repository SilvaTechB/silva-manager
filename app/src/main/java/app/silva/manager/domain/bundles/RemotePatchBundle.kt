package app.silva.manager.domain.bundles

import app.silva.manager.domain.manager.PreferencesManager
import app.silva.manager.network.api.SilvaAPI
import app.silva.manager.network.dto.SilvaAsset
import app.silva.manager.network.service.HttpService
import app.silva.manager.network.utils.getOrThrow
import app.silva.manager.util.ChangelogEntry
...
sealed class RemotePatchBundle(
    ...
    protected abstract suspend fun getLatestInfo(): SilvaAsset
    abstract fun copy(
    ...
    protected open suspend fun download(info: SilvaAsset, onProgress: PatchBundleDownloadProgress? = null) =
    ...
    suspend fun fetchLatestReleaseInfo(): SilvaAsset {
    ...
    protected suspend fun fetchAndCacheEntries(
        ...
        return if (sinceVersion != null)
            app.silva.manager.util.ChangelogParser.entriesNewerThan(allEntries, sinceVersion)
        else allEntries
    }
...
class JsonPatchBundle(
    ...
    override suspend fun getLatestInfo() = withContext(Dispatchers.IO) {
        ...
        val asset = http.request<SilvaAsset> {
...
class APIPatchBundle(
    ...
    private val api: SilvaAPI by inject()
...
class GitHubPullRequestBundle(
    ...
    override suspend fun download(info: SilvaAsset, onProgress: PatchBundleDownloadProgress?) = withContext(Dispatchers.IO) {
...
private data class CachedChangelog(val asset: SilvaAsset, val timestamp: Long)
